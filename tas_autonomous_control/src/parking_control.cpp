...
int parking_procedure(parking *einparken)
{
	...
	if (side == 1) // If parking slot is on the left side:
	{
		...
		car_state += 2 * (fabs(start_orientation - einparken->orientation) < 0.85 && (start_orientation - einparken->orientation) > 0.75); // 0 = car is oblique; 1 = car is in 45° differnt to starting position
	...
	else if (side == -1) // If parking slot is on the right side:
	{
		//LSB
		car_state += 1 * ((start_orientation - einparken->orientation) < 0.04 && (start_orientation - einparken->orientation) > -0.04); // 0 = car orientation not parallel to wall;  1 = car orientation parallel to wall
		car_state += 2 * (fabs(start_orientation - einparken->orientation) < 0.85 && fabs(start_orientation - einparken->orientation) > 0.75); // 0 = car is oblique; 1 = car is in 45° differnt to starting position
	...
	switch (car_state) // Determine from the CAR_STATE what to do with the car:
	{
		...
		case 2: case 6: case 22: case 18: // 00010, 10010, 00110, 10110
