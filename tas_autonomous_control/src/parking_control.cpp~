...
//ROS_INFO("links1: %f", angle_new_l );
	if (side == 0 || side == -1){ angle_new_r = lese_winkel( 1, 178, 2, einparken); } // Rechte Seite hinten auslesen.
...
//ROS_INFO("links_summe: %f", einparken->edge_detector_l[1] );
//ROS_INFO("rechts_summe: %f", einparken->edge_detector_r[1] );
...
int parking_procedure(parking *einparken)
{
	...
	if (side == 1) // Wenn Parklücke auf linker Seite:
	{
		...
		car_state += 4 * ((lese_winkel( 1, 40, 10, einparken) + lese_winkel( 0, 130, 10, einparken)) < 1.0); // 0 = außerhalb Parklücke; 1 = innerhalb Parklücke
	...
	else if (side == -1) // Wenn Parklücke auf rechter Seite:
	{
		...
		car_state += 1 * ((start_orientation - einparken->orientation) < 0.04 && (start_orientation - einparken->orientation) > -0.04); // 0 = Auto steht schief;  1 = Auto steht gerade
		car_state += 2 * (fabs(start_orientation - einparken->orientation) < 0.43 && fabs(start_orientation - einparken->orientation) > 0.36); // 0 = Auto steht schief; 1 = Auto steht im 45° Winkel
		car_state += 4 * ((lese_winkel( 0, 40, 10, einparken) + lese_winkel( 1, 130, 10, einparken)) < 1.0); // 0 = außerhalb Parklücke; 1 = innerhalb Parklücke
		car_state += 8 * (lese_winkel( 1, 115, 40, einparken) < 0.65); // 0 = weit weg von der Wand; 1 = nahe der Wand
...
ROS_INFO("back/front rechts = %f", lese_winkel( 1, 125, 5, einparken)/lese_winkel( 0, 50, 5, einparken));
	...
	switch (car_state) // Entscheide aufgrund der momentanen Position des Autos, was zu tun ist:
	{
		...
		case 4: case 5: case 12: case 13:  // 00100, 00101, 01100, 01101
			...
			{
				if ((lese_winkel( 1, 50, 5, einparken)/lese_winkel( 0, 125, 5, einparken) < 0.9 && side == 1) || (lese_winkel( 1, 125, 5, einparken)/lese_winkel( 0, 50, 5, einparken) < 0.9 && side == -1 )) // Wenn Auto zu weit hinten steht:
				{
					einparken->cmd_parking.linear.x = 0.10;
					velocity_old = einparken->cmd_parking.linear.x;
				}
				else if ((lese_winkel( 1, 50, 5, einparken)/lese_winkel( 0, 125, 5, einparken) > 1.1 && side == 1) || (lese_winkel( 1, 125, 5, einparken)/lese_winkel( 0, 50, 5, einparken) > 1.1 && side == -1 )) // Wenn Auto zu weit vorne steht:
				{
					einparken->cmd_parking.linear.x = -0.10;
					velocity_old = einparken->cmd_parking.linear.x;
				}
...
float lese_winkel(int array, int winkel_start, int winkel_diff, parking *einparken){
	...
	//ROS_INFO("start %i | diff %i | laenge %f ", winkel_start, winkel_diff, winkel_array[0]);


    for (int i =1; i < winkel_diff; i++){
		if (isinf(winkel_array[winkel_start + i]) == 0 && isnan(winkel_array[winkel_start + i]) == 0 && winkel_array[winkel_start + i] < 10 && winkel_array[winkel_start + i] > 0) // Wenn Werte keine Infs oder NaNs und sinnvoll sind:
		{
