...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
	{
        ...
        if (einparken.park)
		{
			if (einparken.start1 == true)  // Merkt sich die Orientierung Anfang - sollte parallel zur Wand sein. Einmalige Aktion
			{
				start_orientation = einparken.orientation; 
start_orientation = 0.48; //test
				einparken.start1 = false;
				ROS_INFO("Start orientation = %f", start_orientation);
			}
			if (einparken.detect_edge < 3)// = 3, wenn Auto hinter der Parklücke steht und bereit ist einzuparken.
			{
...
int parking_procedure(parking *einparken)
{
	int car_state = 0;
//LSB
	car_state += 1 * (((start_orientation - einparken->orientation) < 0.05) && ((start_orientation - einparken->orientation) > -0.05)); // 0 = Auto steht schief;  1 = Auto steht gerade ???????????????
	car_state += 2 * (((start_orientation - einparken->orientation) < 0.45) && ((start_orientation - einparken->orientation) > 0.35)); // 0 = Auto steht schief; 1 = Auto steht im 45° Winkel ?????????
	car_state += 4 * ((lese_winkel( einparken->range_array_back, 1, 20) + lese_winkel( einparken->range_array_front, 140, 20)) < 0.7); // 0 = außerhalb Parklücke; 1 = innerhalb Parklücke
	car_state += 8 * (lese_winkel( einparken->range_array_back, 20, 50) < 0.55); // 0 = weit weg von der Wand; 1 = nahe der Wand
	car_state += 16 * (lese_winkel( einparken->range_array_back, 1, 40)/lese_winkel( einparken->range_array_front, 120, 40) > 0.85 && lese_winkel( einparken->range_array_back, 1, 20)/lese_winkel( einparken->range_array_front, 140, 20) < 1.15); // 0 = Abstand vorne != Abstand hinten; 1 = Abstand vorne = Abstand hinten
//MSB
...
ROS_INFO("back/front = %f", lese_winkel( einparken->range_array_back, 1, 20)/lese_winkel( einparken->range_array_front, 140, 20));
	...
	switch (car_state)
	{
		...
		case 5: case 13: case 21: case 29: // 00101, 01101, 10101, 11101
			if (lese_winkel( einparken->range_array_back, 1, 40)/lese_winkel( einparken->range_array_front, 120, 40) < 0.9 )
			{
				einparken->cmd_parking.linear.x = 0.10;
				einparken->cmd_parking.angular.z = 0.0;
				set_cmd_vel(einparken->cmd_parking, einparken);	
			}
			else if (lese_winkel( einparken->range_array_back, 1, 40)/lese_winkel( einparken->range_array_front, 120, 40) > 1.1)
			{
				einparken->cmd_parking.linear.x = -0.10;
				einparken->cmd_parking.angular.z = 0.0;
				set_cmd_vel(einparken->cmd_parking, einparken);	
